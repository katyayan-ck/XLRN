        ...
        <div class="card">
            <div
                ...
                <div class="d-flex align-items-center gap-3 flex-nowrap">
                    <a href="{{ backpack_url('garage/create') }}" class="btn btn-blue btn-sm fw-bold shadow-sm">
                        <i class="la la-plus me-1"></i> Add New Garage
                    </a>
                </div>
            </div>
...
<script>
    ...
    const columnDefs = [
        {
            ...
            children: ALL_COLUMNS.filter(col => ['serial_no', 'name', 'person_name'].includes(col.field))
        },
        {
            headerName: 'Location',
            headerClass: 'center-header',
            children: ALL_COLUMNS.filter(col => ['address', 'city', 'state'].includes(col.field))
        },
        {
            headerName: 'Contact',
            headerClass: 'center-header',
            children: ALL_COLUMNS.filter(col => ['mobile'].includes(col.field))
        },
        {
            headerName: 'Status',
            headerClass: 'center-header',
    ...
    const gridOptions = {
        ...
        onGridReady: params => {
            gridApi = params.api;
            const defaultFields = ['serial_no', 'name', 'person_name', 'city', 'mobile', 'is_active', 'action'];
            const allCols = [];
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('exportCsv').addEventListener('click', () => {
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Garages");
            XLSX.writeFile(wb, `garages-${new Date().toISOString().slice(0,10)}.xlsx`);
        });
        ...
        document.getElementById('exportPdf').addEventListener('click', () => {
            ...
            doc.save(`garages-${new Date().toISOString().slice(0,10)}.pdf`);
        });
    });
</script>
@endpush