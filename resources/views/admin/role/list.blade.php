...
<script>
    ...
    const columnDefs = [
        ...getCols(['serial_no', 'name']).map(col => {
        ...
        ...getCols(['guard_name', 'permissions_count']),

        ...getCols(['is_active']).map(col => {
            col.cellRenderer = params => {
                if (params.value === 1 || params.value === true) {
                    return `<span class="badge bg-success px-3 py-1">Active</span>`;
                }
                return `<span class="badge bg-secondary px-3 py-1">Inactive</span>`;
            };
            return col;
        }),
    ...
    const gridOptions = {
        ...
        onGridReady: params => {
            ...
            const defaultFields = ['serial_no', 'name', 'guard_name', 'permissions_count', 'is_active', 'action'];
    ...
    function openColumnBubble() {
        ...
        allFlatColumns.forEach(col => {
            ...
            if (['serial_no', 'name'].includes(col.field)) {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('exportCsv').addEventListener('click', () => {
            ...
            XLSX.writeFile(wb, `roles-${new Date().toISOString().slice(0,10)}.xlsx`);
        });
        ...
        document.getElementById('exportPdf').addEventListener('click', () => {
            ...
            doc.save(`roles-${new Date().toISOString().slice(0,10)}.pdf`);
        });
    });
</script>
@endpush
        ...
        <div class="card">
            ...
            <div
                ...
                <div class="d-flex align-items-center gap-3 flex-nowrap">
                    <a href="{{ backpack_url('role/create') }}" class="btn btn-blue btn-sm fw-bold shadow-sm">
                        <i class="la la-plus me-1"></i> Add New Role
                    </a>
                </div>
            </div>
