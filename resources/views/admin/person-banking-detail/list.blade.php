...
<script>
    ...
    const columnDefs = [
        {
            ...
            children: ALL_COLUMNS.filter(col => ['serial_no', 'person_name', 'account_holder_name'].includes(col.field))
        },
        {
            headerName: 'Bank Details',
            headerClass: 'center-header',
            children: ALL_COLUMNS.filter(col => ['bank_name', 'account_number', 'ifsc_code', 'account_type'].includes(col.field))
        },
        {
            headerName: 'Status',
            headerClass: 'center-header',
            children: ALL_COLUMNS.filter(col => ['is_primary', 'is_verified'].includes(col.field))
        },
        {
            headerName: 'Actions',
            headerClass: 'center-header',
    ...
    const gridOptions = {
        ...
        onGridReady: params => {
            gridApi = params.api;
            const defaultFields = ['serial_no', 'person_name', 'account_holder_name', 'bank_name', 'account_number', 'ifsc_code', 'action'];
            const allCols = [];
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('exportCsv').addEventListener('click', () => {
            ...
            XLSX.utils.book_append_sheet(wb, ws, "PersonBankingDetails");
            XLSX.writeFile(wb, `person-banking-details-${new Date().toISOString().slice(0,10)}.xlsx`);
        });
        ...
        document.getElementById('exportPdf').addEventListener('click', () => {
            ...
            doc.save(`person-banking-details-${new Date().toISOString().slice(0,10)}.pdf`);
        });
    });
</script>
@endpush